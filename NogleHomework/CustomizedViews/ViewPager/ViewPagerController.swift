...
public protocol ViewPagerControllerDataSource: AnyObject {
...
public class ViewPagerController:UIViewController {
    ...
    fileprivate var isTabTapped = false
    ...
    fileprivate func setupTabContainerView() {
        ...
        tabContainer = UIScrollView(frame: CGRect(x: options.tabViewMarginLeft, y:0, width: viewPagerFrame.width - options.tabViewMarginLeft - options.tabViewMarginRight, height: options.tabViewHeight))
//        tabContainer.backgroundColor = options.tabViewBackgroundDefaultColor
        ...
        let metrics:[String:CGFloat] = [
            "tabViewMarginTop":options.tabViewMarginTop,
            "tabViewMarginLeft":options.tabViewMarginLeft,
            "tabViewMarginRight":options.tabViewMarginRight,
            "tabViewHeight":options.tabViewHeight
        ]
        self.view.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-(tabViewMarginLeft)-[v0]-(tabViewMarginRight)-|", options: NSLayoutConstraint.FormatOptions(), metrics: metrics, views: viewDict))
    ...
    fileprivate func setupTabs() {
        ...
        if options.fitAllTabsInView {
            ...
            let tabContainerFrame = tabContainer.bounds
            
            // Calculating width for each tab
            let eachLabelWidth = (tabContainerFrame.width / CGFloat (tabsList.count))
    ...
    fileprivate func setupCurrentPageIndicator(currentIndex: Int, previousIndex: Int) {
        ...
        var nextTabFrame: CGRect? = nil
        if currentIndex > previousIndex && currentIndex+1 < tabsViewList.count {
            nextTabFrame = tabsViewList[currentIndex+1].frame
        }
        else if currentIndex < previousIndex && currentIndex-1 >= 0 {
            nextTabFrame = tabsViewList[currentIndex-1].frame
        }
        ...
        if options.isTabIndicatorAvailable {
            ...
            let indicatorWidth = options.tabIndicatorViewWidth < 0 ? currentTabFrame.width : options.tabIndicatorViewWidth
            let indicatorHeight = options.tabIndicatorViewHeight
            let xPosition:CGFloat = currentTabFrame.origin.x + currentTabFrame.width / 2 - indicatorWidth / 2
            ...
            UIView.animate(withDuration: 0.1, animations: {
                ...
                if !self.options.fitAllTabsInView {
                    let frame = currentTabFrame
                    let offsetCenterX = self.tabContainer.contentOffset.x + self.tabContainer.frame.size.width/2.0
                    let tabCenterX = frame.origin.x + frame.size.width/2.0
                    let distance = tabCenterX - offsetCenterX
                    var offset = distance < 0 ? -distance : distance
                    if distance > 0 {
                        if self.tabContainer.contentOffset.x + self.tabContainer.frame.size.width + offset > self.tabContainer.contentSize.width {
                            offset = self.tabContainer.contentSize.width - (self.tabContainer.contentOffset.x + self.tabContainer.frame.size.width)
                        }
                        self.tabContainer.contentOffset = CGPoint(x: self.tabContainer.contentOffset.x + offset, y: 0)
                    }
                    else {
                        if self.tabContainer.contentOffset.x < offset {
                            offset = self.tabContainer.contentOffset.x
                        }
                        self.tabContainer.contentOffset = CGPoint(x: self.tabContainer.contentOffset.x - offset, y: 0)
                    }
                }
        ...
        UIView.animate(withDuration: 0.1) {
            let frame = nextTabFrame ?? currentTabFrame
            self.tabContainer.scrollRectToVisible(frame, animated: false)
        }
    }
    ...
    fileprivate func setupPageControllerFrame() {
        ...
        let viewPagerFrame = options.viewPagerFrame
        let viewPagerHeight = options.viewPagerFrame.height - options.tabViewHeight - options.tabViewMarginBottom - options.viewPagerFrame.origin.y
        
        let pageControllerFrame = CGRect(x: viewPagerFrame.origin.x, y: options.tabViewHeight + options.tabViewMarginBottom, width: viewPagerFrame.width, height: viewPagerHeight)
    ...
    public func displayViewController(atIndex index:Int) {
        ...
        setupCurrentPageIndicator(currentIndex: index, previousIndex: previousIndex)
...
extension ViewPagerController: UIPageViewControllerDelegate, UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if isTabTapped {
            if scrollView.contentOffset.x == options.viewPagerFrame.width {
                isTabTapped = false
            }
            return
        }
...
//        print(options.viewPagerFrame.width)
        let offsetRatio = (scrollView.contentOffset.x - options.viewPagerFrame.width) / options.viewPagerFrame.width
        if offsetRatio != 0 {
            var offSet: CGFloat = 0
            let currentTabFrame = tabsViewList[currentPageIndex].frame
            let oringinX = currentTabFrame.origin.x + ((currentTabFrame.width - tabIndicator.frame.width) / 2)
            var frame = tabIndicator.frame
            
            var nextTabFrame: CGRect? = nil
            if offsetRatio > 0 && currentPageIndex+1 < tabsViewList.count {
                nextTabFrame = tabsViewList[currentPageIndex+1].frame
            }
            else if offsetRatio < 0 && currentPageIndex-1 >= 0 {
                nextTabFrame = tabsViewList[currentPageIndex-1].frame
            }
            
            if let nextFrame = nextTabFrame {
                offSet = abs(currentTabFrame.origin.x - nextFrame.origin.x) * offsetRatio
                let currentTabWidth = currentTabFrame.size.width
                let nextTabWidth = nextFrame.size.width
                if options.tabIndicatorViewWidth < 0 {
                    let width = (nextTabWidth - currentTabWidth) * abs(offsetRatio)
                    frame.size.width = currentTabWidth + width
                }
            }
            
            frame.origin.x = oringinX + offSet
