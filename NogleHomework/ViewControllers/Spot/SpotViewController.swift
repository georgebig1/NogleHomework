...
class SpotViewController: UIViewController {
    static func create() -> SpotViewController {
        return SpotViewController()
    }
    
    let vm = SpotViewModel()
    ...
    func initBinding() {
        vm.spotList.subscribeSuccess {[weak self] _ in
...
extension SpotViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return vm.spotList.value.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let item = vm.spotList.value[indexPath.row]
